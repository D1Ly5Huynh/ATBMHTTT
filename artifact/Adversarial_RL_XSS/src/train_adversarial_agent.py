...
def train_adversarial_agent(opt):
    ...
    os.makedirs(save_folder, exist_ok=True)
    n_eval = opt.n_eval_episodes if opt.n_eval_episodes else len(validation_set)
    ...
    model.save(os.path.join(save_folder, "last_model"))
...
def add_parse_arguments(parser):
    ...
    parser.add_argument('--n_eval_episodes', type=int, default=0, help='0 = len(val) like paper; 32 is enough to pick best_model')
    parser.add_argument('--skip_env_check', action="store_true", help='skip gymnasium check_env')
