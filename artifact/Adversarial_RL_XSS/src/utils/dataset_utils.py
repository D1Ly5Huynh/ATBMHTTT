...
def check_column_validity_with_oracle(df, column, endpoint):
    # Paper refetches "abc" on every call. Same DOM baseline — cache it.
    basic_html = _ABC_HTML.get(endpoint)
    if basic_html is None:
        basic_html = do_xss_post_request(endpoint, "abc")
        _ABC_HTML[endpoint] = basic_html
